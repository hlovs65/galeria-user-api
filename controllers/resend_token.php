...
require_once '../includes/send_email.php'; // Incluir funciones de envío de correos electrónicos
...
// ----------------------------------------------------
    ...
    $columna_to_search = 'email';
    $valor_to_search = $email;

    $user_data_from_db = get_user_data_by_field($conn, $columna_to_select, $columna_to_search, $valor_to_search);

    if ($user_data_from_db === false || $user_data_from_db === null) {
        if (isset($conn) && $conn->ping()) { $conn->close(); }
        send_json_response(200, [
            "status" => "error",
            "message" => "El correo electrónico no está registrado."
        ]);
    } else if ($user_data_from_db['correo_verificado'] == 1) {
        if (isset($conn) && $conn->ping()) { $conn->close(); }
        send_json_response(200, [
            "status" => "success",
...
// ----------------------------------------------------
    ...
    $correo_enviado = email_verification($conn, $email, $user_id, $name_link, $name_table, $verification_subject, $verification_message);
...
// ----------------------------------------------------
    ...
    if ($correo_enviado) {
        ...
        send_json_error("Error al enviar el correo de verificación.\n\nPor favor, inténtalo de nuevo más tarde.", 500);
    }

} else {
    // Si la solicitud no es POST, redirigir al formulario para solicitar un nuevo token
    if (isset($conn) && $conn->ping()) { $conn->close(); }
    send_json_error("Solicitud no válida. Por favor, utiliza el formulario para solicitar un nuevo token.", 405);
}
